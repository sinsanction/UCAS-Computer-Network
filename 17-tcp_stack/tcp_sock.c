...
struct tcp_sock *tcp_sock_lookup_established(u32 saddr, u32 daddr, u16 sport, u16 dport)
{
	...
	list_for_each_entry(tmp, list, hash_list) {
		if (saddr == tmp->sk_sip && daddr == tmp->sk_dip && sport == tmp->sk_sport && dport == tmp->sk_dport)
			return tmp;
	}
...
{
	send_buffer_entry_t *sb_entry = (send_buffer_entry_t *)malloc(sizeof(send_buffer_entry_t));
	memset(sb_entry, 0, sizeof(send_buffer_entry_t));

	sb_entry->packet = (char *)malloc(len);
	sb_entry->len = len;
	memcpy(sb_entry->packet, packet, len);
	init_list_head(&sb_entry->list);
	list_add_tail(&sb_entry->list, &tsk->send_buf);
}

int tcp_delete_send_buffer(struct tcp_sock *tsk, u32 ack)
{
	int flag = 0;
